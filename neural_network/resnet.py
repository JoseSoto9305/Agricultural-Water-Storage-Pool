...
def ResNet50_FCN(input_shape=(224,224,3), n_classes=2, 
    ...
    # Get the final layers: 32X32, 16X16 and 8X8
    x32 = model.get_layer('act3d').output
    x16 = model.get_layer('act4f').output
    x8 = model.get_layer('act5c').output
    
    # Compress the skip connections to the n_classes channels
    c32 = Conv2D(n_classes, (1, 1), name='conv_labels_32')(x32)
    c16 = Conv2D(n_classes, (1, 1), name='conv_labels_16')(x16)
    c8 = Conv2D(n_classes, (1, 1), name='conv_labels_8')(x8)
    
    def resize_bilinear(images):
        return tf.image.resize_bilinear(images, [nb_rows, nb_cols])
    
    # Upsampling the layer
    ...
    # Sum the skip connections upsampled
    m = Add(name='merge_labels')([r32, r16, r8])
    
    # Softmax activation
    x = Reshape((nb_rows * nb_cols, n_classes))(m)
    x = Activation('softmax')(x)
    #x = Reshape((nb_rows, nb_cols, n_classes))(x)
    
    model_fcn = Model(inputs=input_tensor, outputs=x)
    return model_fcn
